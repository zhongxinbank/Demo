...
class SmartTopicAnalyser:
    ...
    def create(cls, matched_sentences, prob, target, analyser_type, host="localhost", port=3306, user="helic",
        ...
        try:
            ...
            sql = "INSERT INTO smart_topic_analyser(id, matched_sentences, prob, target, analyser_type) VALUES ('%s', '%s', %f, '%s', '%s')" % (
    ...
    #          charset="utf8"):
    #     """从数据库中读取analyser_id对应的model,返回一个SimTopicAnalyser instance"""
    #     try:
    #         _, cursor = connect_to_db(host=host, port=port, user=user, passwd=passwd, db=db, charset="utf8")
    #         sql = "select * from smart_topic_analyser where id = '%s'" % analyser_id
    #         cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    # SmartTopicAnalyser.create(matched_sentences="就你这种人，送给我我都不要\n麻烦有多远滚多远，越快越好", prob=0.6, target="all",
    #                          analyser_type="测试")

    # analyser1 = SmartTopicAnalyser.load(analyser_id=["11be6f1e-aa16-43d8-935b-e2be21fb8171"])
